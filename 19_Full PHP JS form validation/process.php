...
// check values are set and its variable exists
...
// ajax request/response being defined from js to php
if (isset($_POST['ajaxrequest'])) { $ajaxrequest = $_POST['ajaxrequest']; } else { $ajaxrequest = ''; }
	...
	if (strlen($mypassword) <= 6):
		...
		// passing the ajax request from php to js for error handling etc
  ...
  $salted = hash('sha1', $salty);
  	...
  	// some database submitting 
	if (!($formerrors)) :
		include("log_formdb.php"); // database file is on the server
		...
		if ($forminforesult = mysqli_query($forminfolink, $forminfoquery)): // passing above stuff to the database
